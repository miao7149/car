...
namespace UnityEngine.UI {
    ...
    public class ExtensionsToggle : Selectable, IPointerClickHandler, ISubmitHandler, ICanvasElement {
        ...
        protected override void OnDidApplyAnimationProperties() {
            ...
            if (graphic != null) {
                var oldValue = !Mathf.Approximately(graphic.alpha, 0);
                if (m_IsOn != oldValue) {
                    m_IsOn = oldValue;
                    Set(!oldValue);
                }
        ...
        public void QuickSet(bool on) {
            //if (m_IsOn == on) return;
            m_IsOn = on;

            StartCoroutine(SetShow(on));
            if (on) m_Group.QuickNotifyToggleOn(this);
        }

        private IEnumerator SetShow(bool on) {
            yield return new WaitForSeconds(1);
            graphic.alpha = m_IsOn ? 1f : 0f;
            bg.alpha = m_IsOn ? 0f : 1f;
        }
        ...
        private void PlayEffect(bool instant) {
...
#endif
            {
                ...
                DOTween.To(x => { graphic.alpha = x; }, m_IsOn ? 0 : 1, m_IsOn ? 1 : 0, instant ? 0 : 0.1f).Play();

                DOTween.To(x => { bg.alpha = x; }, m_IsOn ? 1 : 0, m_IsOn ? 0 : 1, m_IsOn ? 0.1f : 0f).Play();
            }
        }
