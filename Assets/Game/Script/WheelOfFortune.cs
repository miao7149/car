...
public class WheelOfFortune : MonoBehaviour {
    ...
    void Start() {
        ...
        for (int i = 0; i < propItems.Length; i++) {
            propItems[i].transform.GetChild(0).GetComponent<Text>().text = "X" + propDatas[i].Count.ToString();
        }
    ...
    void Update() {
        ...
        if (m_NoSpinRoot.activeSelf) {
            ...
            m_NoSpinRoot.transform.GetChild(0).GetComponent<Text>().text = timeSpan.Hours.ToString("00") + "h" + timeSpan.Minutes.ToString("00") + "m";
        }
    }
    ...
    private IEnumerator SpinWheel() {
        ...
        m_RewardImage.transform.GetChild(0).GetComponent<Text>().text = "X" + propDatas[selectedSector].Count.ToString();
        //光环顺时针一直旋转
    ...
    public void SetSpinButtonState() {
        ...
        else if (m_TodaySpinCount > 0) //如果还有抽奖次数
        {
            ...
            m_AdSpinRoot.transform.GetChild(2).GetComponent<Text>().text = m_TodaySpinCount.ToString() + "/5";
            m_NoSpinRoot.SetActive(false);
            m_SpinButton.image.sprite = m_SpinButtonSprites[0];
        }
        else {
